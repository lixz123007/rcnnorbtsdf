        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    //重建
    voxel_grid_origin_x = -2.50f; // Location of voxel grid origin in base frame camera coordinates
    voxel_grid_origin_y = -2.50f;
    voxel_grid_origin_z = 0.05f;
    voxel_size = 0.01f;
...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp,const int times)
{
    ...
    for (int c = 0; c <depthmap.cols ; ++c) {
     //5-12mask//if(black.ptr<uchar>(r)[c]!=0){continue;}//||black.at<Vec3b>(r, c)[1]!=0||black.at<Vec3b>(r, c)[2]!=0)continue;//mask way to do!cout<<r<<"r c: "<<c<<" result:"<<black.ptr<uchar>(r)[c]<<endl;
      ...
      depth[r * depthmap.cols + c] = (float)(depthmap.at<unsigned short>(r, c)) /5208.0f;
     // if(mbRGB)
      ...
      */
      ...
      if (depth[r *depthmap.cols+ c] > 6.0f&&depth[r *depthmap.cols+ c] < 0.05f) // Only consider depth < 6m
       {
...
void System::SaveMap(){
  ...
  FILE *fp = fopen("/home/finch/savingmesh/map.ply", "w");
  fprintf(fp, "ply\n");
  ...
  std::ofstream outFile("/home/finch/savingmesh/map.bin", std::ios::binary | std::ios::out);
